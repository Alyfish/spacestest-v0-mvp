...
class _PreferredStoresContentState extends State<PreferredStoresContent> {
  ...
  bool _didTriggerContinueNavigation = false;
    ...
    try {
      ...
      setState(() {
        ...
        if (existingSelection.isNotEmpty) {
          _selectedStoreIds = existingSelection
              .map((name) {
                final match = stores.firstWhere(
                  (s) => s.name == name || s.id == name,
                  orElse: () =>
                      const PreferredStore(id: '', name: '', logoUrl: ''),
                );
                return match.id;
              })
  ...
  Future<void> _handleContinue() async {
    if (_didTriggerContinueNavigation) return;
    if (_isLoading) return;
    if (widget.onContinue == null && _isSaving) return;
    final projectProvider = Provider.of<ProjectProvider>(
      context,
      listen: false,
    );
    ...
    // Map store IDs to display names for the backend
    final storeNames = _selectedStoreIds
        .where((id) => _stores.any((s) => s.id == id))
        .map((id) => _stores.firstWhere((s) => s.id == id).name)
        .toList();

    if (widget.onContinue != null) {
      _didTriggerContinueNavigation = true;
      projectProvider.setPreferredStoresLocal(storeNames);
      widget.onContinue!();
      return;
    }
  ...
  Widget _buildFallbackLogo() {
    return Container(
      ...
      child: Icon(Icons.store_outlined, size: 32, color: AppTheme.textTertiary),
    );
  }

  Widget _buildBottomButtons() {
    final hasSelection = _selectedStoreIds.isNotEmpty;
    final showBlockingSaveState = widget.onContinue == null && _isSaving;
            ...
            Expanded(
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: showBlockingSaveState ? null : _handleContinue,
                  child: showBlockingSaveState
                      ? const SizedBox(
                          width: 22,
                          height: 22,
