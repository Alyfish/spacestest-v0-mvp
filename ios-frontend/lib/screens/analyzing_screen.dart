...
class _AnalyzingScreenState extends State<AnalyzingScreen>
  ...
  late Animation<double> _progressBarOpacity;
  ...
  void _initAnimations() {
    ...
    _iconControllers = List.generate(
      _icons.length,
      (index) =>
          AnimationController(vsync: this, duration: _iconAppearDuration),
    );
    ...
    // Progress bar fades in after a brief delay (matches first icon fade-in)
    _progressBarController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 600),
    );
    _progressBarOpacity = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _progressBarController, curve: Curves.easeOut),
    );
  }

  void _startStaggeredAnimation() async {
    // Show progress bar alongside the first icon
...
class _LoadingIcon {
  ...
  const _LoadingIcon({required this.icon, required this.label});
}
